...
const GolfEventModal = ({ open, onClose, golfEvent, onEdit }) => {
  ...
  useEffect(() => {
    ...
    async function getPlayersForEvent(eventId) {
      const response = await fetch(
        `${process.env.REACT_APP_API}/events/getPlayersForEvent/${eventId}`
      );
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API}/events/updateEvent/${editedGolfEvent.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const removePlayerFromEvent = async (eventId, playerId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API}/events/deletePlayerFromEvent/${eventId}/${playerId}`,
        {
          method: "DELETE",
        }
      );
