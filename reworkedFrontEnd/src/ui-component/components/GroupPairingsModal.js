...
const GroupPairingsModal = ({ open, onClose, golfEvent }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      fetch(`${process.env.REACT_APP_API}/events/getPlayersForEvent/${golfEvent.id}`)
      ...
      fetch(`${process.env.REACT_APP_API}/events/getplayergroups/${golfEvent.id}`)
        .then((response) => response.json())
        .then((data) => {
  ...
  const handleSaveGroups = async () => {
    ...
    fetch(`${process.env.REACT_APP_API}/events/savegroupsforevent`, {
      method: "POST",
      headers: {
