...
public interface CompetitionRepository extends JpaRepository<Competition, Long> {
	 ...
	 @Query(value = "SELECT * FROM competition WHERE CASE WHEN :searchType = 'id' THEN id = CAST(:searchText AS UNSIGNED) WHEN :searchType = 'name' THEN name LIKE CONCAT('%', :searchText, '%') ELSE FALSE END", nativeQuery = true)
